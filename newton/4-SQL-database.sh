...
eth0=$1
if [ ! -f ${ch_stat} ];then
	echo "mysql-server mysql-server/root_password password $2" | /usr/bin/debconf-set-selections
	echo "mysql-server mysql-server/root_password_again password $2" | /usr/bin/debconf-set-selections
	echo "y" | apt-get install mariadb-server python-pymysql
	...
	sed -i "/bind-address/,+0amax_connections = 4096" ${confile}
	sed -i "/bind-address/,+0ainnodb_file_per_table" ${confile}

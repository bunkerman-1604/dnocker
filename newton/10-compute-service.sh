...
if [ ! -f /var/log/nova_stat ];then
	...
	echo "y" | apt-get install nova-api nova-conductor nova-consoleauth nova-novncproxy nova-scheduler
...
sed -i "/\[DEFAULT\]/,+0atransport_url = rabbit://openstack:${RABBIT_PASS}@${CTL_HOST}" /etc/nova/nova.conf

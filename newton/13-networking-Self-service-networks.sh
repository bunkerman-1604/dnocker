...
if [ ! -f ${ch_stat} ];then
	...
	echo "y" | apt-get install neutron-server neutron-plugin-ml2 neutron-linuxbridge-agent neutron-l3-agent neutron-dhcp-agent neutron-metadata-agent
fi
...
sed -i "/^\[DEFAULT\]/,+0atransport_url = rabbit://openstack:${RABBIT_PASS}@${CTL_HOST}" /etc/neutron/neutron.conf
